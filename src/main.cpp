...
#if defined(NDEBUG)
# error "Billionairecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x75304434f08e88b24cb37a40d2dd6ec477f3e31777b069ab9cee0f0ce3bcb534");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Billionairecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Billionairecoin Signed Message:\n";
...
{
    // Billionairecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 155 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 700000); // Billionairecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.14159265359 * 24 * 60 * 60; // Billionairecoin: 3.14159265359 days
static const int64 nTargetSpacing = 7 * 60; // Billionairecoin: 7 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Billionairecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Billionairecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9fdae1eb829128369ca82973f241e38dd9fa5c516c07792f3e97bc6b402b90b5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Billionaairecoin -owner- Mobin Alimohammadi";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 155 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b56d701f49ab8cdc20a5209d60e8dd94ed0470e7f1629896eaaad92bc8f923f2bc8d4f5375e5482f398936b2d293ad74be0347850a5d7cb5f5718723686d5d93") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1555415726;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084538049;

        if (fTestNet)
        {
            block.nTime    = 1555415793;
            block.nNonce   = 385720511;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xfdcc50da1789446670b6a9310afc15eac6e334bbd5f6c2b07adf3bdadf2fb7b5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc4, 0xbc, 0xdf }; // Billionairecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BillionairecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BillionairecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BillionairecoinMiner(CWallet *pwallet)
{
    printf("BillionairecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Billionairecoin-miner");
    ...
    try { loop {
        ...
        printf("Running BillionairecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BillionairecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BillionairecoinMiner, pwallet));
}
